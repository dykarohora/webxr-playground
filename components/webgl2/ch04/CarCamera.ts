...
export class CarCamera {
  ...
  private lightPosition: [number, number, number] = [100, 100, 100]
  ...
  private configure(canvas: HTMLCanvasElement) {
    ...
    this.camera.goHome([0, 20, 120])

    new Controls(this.camera, canvas)

    this.gl.uniform4fv(this.program.getUniformLocation('uLightAmbient'), [0.1, 0.1, 0.1, 1])
    this.gl.uniform3fv(this.program.getUniformLocation('uLightPosition'), this.lightPosition)
    this.gl.uniform4fv(this.program.getUniformLocation('uLightDiffuse'), [0.7, 0.7, 0.7, 1])
  }
  ...
  private async load() {
    ...
    await this.scene.loadByParts('/common/models/nissan-gtr/part', 178)
  }
